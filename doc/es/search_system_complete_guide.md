...
这是文本向量检索中的一个重要技术决策。让我们对比两种方法：

| 对比项 | 余弦相似度（Cosine Similarity） | 欧氏距离（Euclidean Distance） |
|--------|-------------------------------|-------------------------------|
| **计算公式** | `cos(θ) = (A·B) / (||A|| × ||B||)` | `d = √Σ(Ai - Bi)²` |
| **关注点** | 向量的**方向**（角度） | 向量的**位置**（距离） |
| **归一化** | ✅ 自动归一化，不受向量长度影响 | ❌ 受向量长度影响 |
| **范围** | [-1, 1]，1 表示完全相同 | [0, +∞)，0 表示完全相同 |
| **文本语义** | ✅ 更适合，语义主要体现在方向 | ⚠️ 不适合，受向量长度影响大 |
| **计算复杂度** | O(n)，需要点积和模长 | O(n)，需要平方和开方 |

**核心原因**：

1. **文本向量的语义在方向上，不在长度上**
   - 例如："修仙小说" 和 "修真小说" 的向量方向相似，但长度可能不同
   - 余弦相似度只关注方向，能准确捕捉语义相似性
   - 欧氏距离会受向量长度影响，可能误判

2. **归一化特性**
   - 余弦相似度天然归一化，不同长度的文本向量可以直接比较
   - 例如：短文本（书名）和长文本（简介）的向量长度不同，但语义可能相似
   - 欧氏距离需要手动归一化，否则长文本向量会占优势

3. **实际示例**
   ```
   向量 A: [0.5, 0.5, 0.5, ...] (长度: 0.866)
   向量 B: [1.0, 1.0, 1.0, ...] (长度: 1.732)
   向量 C: [0.3, 0.3, 0.9, ...] (长度: 0.995)
   
   假设 A 和 B 方向相同（语义相同），C 方向不同（语义不同）
   
   余弦相似度：
   - cos(A, B) = 1.0  ✅ 正确识别为相同
   - cos(A, C) = 0.8  ✅ 正确识别为相似但不相同
   
   欧氏距离（未归一化）：
   - d(A, B) = 较大值  ❌ 误判为不同（因为长度不同）
   - d(A, C) = 较小值  ❌ 可能误判为更相似
   ```

4. **Elasticsearch 的 dense_vector 支持**
   - Elasticsearch 的 `dense_vector` 类型支持 `cosine`、`dot_product`、`l2_norm`（欧氏距离）
   - 对于文本 Embedding，业界普遍推荐使用 `cosine`
   - `l2_norm` 更适合图像、音频等非文本向量

**总结**：
- ✅ **余弦相似度**：适合文本向量，关注语义方向，不受长度影响
- ⚠️ **欧氏距离**：适合位置相关的向量（如图像像素），受长度影响大

因此，在文本搜索场景中，**余弦相似度是更合适的选择**。
